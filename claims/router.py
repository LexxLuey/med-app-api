...
def read_claim(claim_id: str, db: Session = Depends(get_db)):
...
def update_claim(claim_id: str, claim: ClaimCreate, db: Session = Depends(get_db)):
...
@router.delete(
    "/{claim_id}", summary="Delete claim", description="Delete a claim by its ID."
)
def delete_claim_endpoint(claim_id: str, db: Session = Depends(get_db)):
