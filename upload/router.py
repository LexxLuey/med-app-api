...
def log_audit_action(db: Session, action: str, user_id: str, details: str):
    ...
    audit_log = AuditLog(
        action=action, user_id=user_id, timestamp=datetime.utcnow().isoformat(), details=details
    )
    db.add(audit_log)
    db.commit()
...
async def upload_claims_file(
    ...
    try:
        ...
        # Check for claim_id column, auto-generate if missing
        if "claim_id" not in df.columns:
            # Generate claim_id from unique_id + service_date
            df["claim_id"] = df.apply(
                lambda row: f"{row['unique_id']}_{row['service_date'].replace('/', '')}", axis=1
            )
        ...
        if missing_columns:
            raise HTTPException(
                status_code=400, detail=f"Missing required columns: {', '.join(missing_columns)}"
            )
        ...
        for _, row in df.iterrows():
            try:
                ...
                service_date = (
                    pd.to_datetime(row["service_date"]).date()
                ...
                claim_data = {
                    ...
                    "encounter_type": (
                        str(row["encounter_type"]) if pd.notna(row["encounter_type"]) else None
                    ),
                    "service_date": service_date,
                    "national_id": (
                        str(row["national_id"]) if pd.notna(row["national_id"]) else None
                    ),
                    "member_id": str(row["member_id"]) if pd.notna(row["member_id"]) else None,
                    "facility_id": (
                        str(row["facility_id"]) if pd.notna(row["facility_id"]) else None
                    ),
                    "unique_id": str(row["unique_id"]) if pd.notna(row["unique_id"]) else None,
                    "diagnosis_codes": (
                        str(row["diagnosis_codes"]) if pd.notna(row["diagnosis_codes"]) else None
                    ),
                    "service_code": (
                        str(row["service_code"]) if pd.notna(row["service_code"]) else None
                    ),
                    "paid_amount_aed": (
                        float(row["paid_amount_aed"]) if pd.notna(row["paid_amount_aed"]) else None
                    ),
                    "approval_number": (
                        str(row["approval_number"]) if pd.notna(row["approval_number"]) else None
                    ),
                ...
                existing_claim = (
                    ...
                    .filter(MasterTable.claim_id == claim_data["claim_id"])
                    .first()
                )
